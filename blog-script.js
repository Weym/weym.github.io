...
function updateIcon(isDark) {
    const iconSrc = isDark
        ? darkThemeSVG 
        : lightThemeSVG;
    toggleBtn.innerHTML = iconSrc;
}

function updatePaletteNames(isDark) {
    if(isDark) {
...
function togglePaletteNamesAndHex() {
    ...
    const currentNames = Array.from(nameElements).map(el => el.textContent);
    const reversedNames = currentNames.reverse();

    nameElements.forEach((el, index) => {
        el.textContent = reversedNames[index];
    });

    const hexElements = document.querySelectorAll('.palette__hex');
    const currentHexes = Array.from(hexElements).map(el => el.textContent);
    const reversedHexes = currentHexes.reverse();
    hexElements.forEach((el, index) => {
        el.textContent = reversedHexes[index];
    });
}
...
if (savedTheme === 'dark') {
    ...
    updateIcon(true);
    updatePaletteNames(true);
} else {
    updateIcon(false);
    updatePaletteNames(false);
}
